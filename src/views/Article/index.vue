<template>
  <div class="article-container">
    ...
    <div class="article-bottom" v-if="!isLoading && !!article.art_id">
      ...
      ></CollectArticle>
      <LikeArticle :like_count.sync="article.like_count"></LikeArticle>
      <van-icon
        name="share"
        color="#777777"
...
export default {
  ...
  async created () {
    ...
    // loading关闭以后 页面显示正文 但是必须还要写在nextTick
    // 数据更新视图不能立即更新 写在nextTick回调中就可以
    // 因为这个回调就是在视图更新完成才回触发
    this.isLoading = false
    this.$nextTick(() => {
      // 获取正文中全部图片
      const arr = (document.querySelectorAll('.article-content img'))
      if (arr.length === 0) return // 防御代码 更安全
      const arr1 = []// 放图片src值
